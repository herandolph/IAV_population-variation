...
permutations <- read.table(paste0("inputs/",folder,"/2_1_correctedExp_and_infection_DE/single_celltype_permutation_outs.txt"), header = TRUE, sep = ",")
...
for (i in 1:length(celltypes)){
	...
	meta_data_i <- meta_data_i[,c(1:20)]
	meta_data_i <- cbind(meta_data_i, counts)

	## model infection differential expression
	design = model.matrix(~ indiv_ID + counts + infection_status, data = meta_data_i)
	design <- design[, colSums(design != 0) > 0]
	...
	## get permutation outs specific for that cell type
