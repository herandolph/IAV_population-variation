...
setwd(current)
## inputs
meta_data_file <- paste0("inputs/",folder,"/individual_meta_data_for_GE_with_scaledCovars_with_CTC.txt")
pseudobulk_dir <- paste0("inputs/",folder,"/2_1_correctedExp_and_infection_DE/pseudobulk/")
celltypes <- c("allCellTypes")
...
permutations <- read.table(paste0("inputs/",folder,"/2_1_correctedExp_and_infection_DE/PBMC_permutation_outs.txt"), header = TRUE, sep = ",")
...
for (i in 1:length(celltypes)){
	...
	## model infection differential expression adding in cell type composition PCs
	design = model.matrix(~ CTC_PC1 + CTC_PC2 + indiv_ID + infection_status*age_Scale, data = meta_data_i)
	design <- design[, colSums(design != 0) > 0]
	...
	## get permutation out specific for that cell type
	perms <- permutations[, grepl(paste0(cell_type_i), names(permutations))]
...
write.table(numDEgenes_per_CT, paste0(results_dir,"number_of_DEG_by_CT_qvalues_logFCcutoff_PBMCs.txt"), quote = FALSE, sep = ",")
