...
stat_distribution <- function(snp_type, coloc_filter){
	...
	for(j in 1:length(trait_groups)){
		...
		colnames(scFLU)[1] <- "pid"

		## merge with larger dataset
		coloc <- subset(coloc, select = pid)
		scFLU <- subset(scFLU, select = pid)
		all_traits_datasets <- rbind(coloc, scFLU)

		## calculate the number of unique genes that are associated with coloc hits
		obs_genes <- as.character(unique(all_traits_datasets$pid))
		obs_num <- length(unique(all_traits_datasets$pid))
		...
		write.table(permutation_outs, paste0(results_dir,"/",trait_group,"_",snp_type,"_",coloc_filter,"_permutation_outs.txt"), quote = FALSE, row.names = FALSE)
		...
		plot <- ggplot(permutation_outs, aes(null)) +
					...
					ggtitle(paste0("n unique genes = ",obs_num, ", perc null tests >= obs perc popDE genes = ", equal_or_greater/npermutations)) +
		...
		pdf(paste0(results_dir,trait_group,"_",snp_type,"_",coloc_filter,"_distribution.pdf"), width = 4, height = 3)
		print(plot)
		dev.off()
	}
}
