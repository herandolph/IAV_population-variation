...
CORR_TYPE <- "withCorr"
MODEL_FOLDER <- "WOS_associated_QN"
MODEL_TYPE <- c("WOS_associated_QN")
COV_TYPE <- c("canonical_and_dataDriven")

## folders
INPUTS_dir <- paste0("inputs/",folder,"/",MODEL_FOLDER,"/")
OUTOUTS_dir <- paste0("outputs/",folder,"/7_3_",MODEL_FOLDER,"/",CORR_TYPE,"/")
...
for(i in 1:length(MODEL_TYPE)){
	...
	for(j in 1:length(COV_TYPE)){
		...
		write.table(V, paste0(OUTOUTS_dir,MODEL,"_correlationMatrix_",CORR_TYPE,"_",COV,"COV.txt"), quote = FALSE)

		## set up the canonical covariance matrix
		U.c = cov_canonical(data_V)
		saveRDS(U.c, paste0(OUTOUTS_dir,MODEL,"_U_c_covarianceMatrices_",CORR_TYPE,"_",COV,"COV.rds"))
		...
		saveRDS(U.pca, paste0(OUTOUTS_dir,MODEL,"_covariancePCAs_",CORR_TYPE,"_",COV,"COV.rds"))
		saveRDS(U.ed, paste0(OUTOUTS_dir,MODEL,"_U_ed_covarianceMatrices_",CORR_TYPE,"_",COV,"COV.rds"))
		...
		write.table(m.pairwise_PM, paste0(OUTOUTS_dir,MODEL,"_pairwise_sharing_PM_",CORR_TYPE,"_",COV,"COV.txt"), quote = FALSE)

		pdf(paste0(OUTOUTS_dir,MODEL,"_pairwiseSharing_sigbyPosteriorMeans_",CORR_TYPE,"_",COV,"COV.pdf"), width=7, height=7)
		...
		pdf(paste0(OUTOUTS_dir,MODEL,"_mixtureProportions_piEstimates_",CORR_TYPE,"_",COV,"COV.pdf"), width=8, height=8)
		par(mar = c(10,4,8,4))
		barplot(get_estimated_pi(m), las = 2)
		dev.off()

		## write posterior outs
		write.table(get_lfsr(m), paste0(OUTOUTS_dir,MODEL,"_lfsr_",CORR_TYPE,"_",COV,"COV.txt"), quote = FALSE)
		write.table(get_pm(m), paste0(OUTOUTS_dir,MODEL,"_posteriorMeans_PM_",CORR_TYPE,"_",COV,"COV.txt"), quote = FALSE)
		write.table(get_psd(m), paste0(OUTOUTS_dir,MODEL,"_posteriorStandardDevs_",CORR_TYPE,"_",COV,"COV.txt"), quote = FALSE)
	}
	print(MODEL)
}
